...
def read_from_camera(frame, psm=7):
    """
    Procesa un frame directo de cámara (ROI) y extrae texto usando Tesseract OCR.
    """
    import cv2
    import pytesseract
    import numpy as np
    ...
    alphanumeric = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
    config = f"-c tessedit_char_whitelist={alphanumeric} --psm {psm}"

    # OCR
    text = pytesseract.image_to_string(thresh, config=config)
    return text.strip().upper(), thresh  # devuelve texto y la imagen binarizada
...
def read_from_camera(frame, psm=7):
    ...
    # Aumentar brillo y contraste automáticamente
    ...
    # OCR
    text = pytesseract.image_to_string(thresh, config=config)
    return text.strip().upper(), thresh  # devuelvo texto y la imagen binarizada
